...
//! - **ロールバック戦略**: 目標フレームへの挿入に失敗した場合、1 フレームずつ前進しながら
//!   挿入可能な位置を再帰的に探索する。これにより、占有オブジェクトの末尾付近に
//!   自動的にフォールバック（ロールバック）する。
...
/// 本関数はロールバック先として `frame + 1` を次の候補とし、再帰的に試行する。
    ...
    F: Fn(usize, usize, usize) -> Result<T, E>,
{
    ...
    match try_insert(layer, frame, length_frames) {
        ...
        Err(_) => {
            // 挿入失敗: 1 フレーム前進してロールバック先を再帰的に探索する
